...
GET DATA 
  ...
  /SQL= 'SELECT CU.CUSTOMER, CU.ZZIPCD5 ZIPCODE, SI.ZORD_DATE, COUNT(DISTINCT SI.S_ORD_NUM) ORDERS, SUM(SI.SUBTOTAL_2) SALES ' +
             ...
             'WHERE CU.ZZIPCD5 <> '''' AND ' +
                          ...
                          'SI.ZZCOMFLG = ''Y'' AND ' +
                          'SI.ZORD_DATE BETWEEN {d ''2011-07-01''} AND {d ''2016-06-30''} ' +
...
*Get the orders and sales for the accommodation account and zipcode from the sales invoice table in Teradata.
GET DATA 
  /TYPE=ODBC 
  /CONNECT='DSN=Teradata;DB=PRD_DWH_VIEW;PORT=1025;DBCNL=10.4.165.29;UID=spss;PWD=$-<~*x#N3@!/-!!+'
...
*June 2016.
DATASET ACTIVATE JUN16.

SELECT IF(ORD_MY = 201606).
EXE.

RENAME VARIABLES ORDERS = ORD_0616.
RENAME VARIABLES SALES = SLS_0616.
...
GET DATA 
  ...
  /SQL= 'SELECT CU.CUSTOMER, CU.ZZIPCD5 ZIPCODE, SI.ZORD_DATE, COUNT(DISTINCT SI.S_ORD_NUM) ORDERS, SUM(SI.SUBTOTAL_2) SALES ' +
             ...
             'WHERE CU.ZZIPCD5 <> '''' AND ' +
                          'CU.ACCNT_GRP = ''0001'' AND ' +
                          'SI.ZZCOMFLG = ''Y'' AND ' +
                          'SI.ZORD_DATE BETWEEN {d ''2011-07-01''} AND {d ''2016-06-30''} AND ' +
                          'SI.SALES_OFF = ''E01'' ' +
             'GROUP BY CU.CUSTOMER, CU.ZZIPCD5, SI.ZORD_DATE '.
CACHE.
EXE.
...
RENAME VARIABLES ORDERS = GC_ORD_0616.
RENAME VARIABLES SALES = GC_SLS_0616.
...
VECTOR GIS = ORD_0711 TO SLS_0616.
VECTOR GC = GC_ORD_0711 TO GC_SLS_0616.
...
VECTOR ACT = ORD_0711 TO GC_SLS_0616.
...
SELECT IF(month >= 201107 AND month <= 201606).
EXE.
...
VECTOR MO = ORD_0711 TO ORD_0616.
VECTOR MS = SLS_0711 TO SLS_0616.
VECTOR GCMO = GC_ORD_0711 TO GC_ORD_0616.
VECTOR GCMS = GC_SLS_0711 TO GC_SLS_0616.
VECTOR SD = K_201107 TO K_201606.
...
LOOP #i = 1 TO 60.
   ...
   DO IF(SD(#i) > 0).
      ...
      COMPUTE ADO(#i) = MO(#i) / SD(#i).
      COMPUTE ADS(#i) = MS(#i) / SD(#i).
      COMPUTE ADGCO(#i) = GCMO(#i) / SD(#i).
...
VECTOR TOT_ORD = ORD_0711 TO ORD_0616.

COMPUTE Has_Orders = 0.
FORMATS Has_Orders(F1.0).

LOOP #j = 1 TO 60.

   IF(TOT_ORD(#j) > 0) Has_Orders = 1.
...
DELETE VARIABLES K_201107 TO K_201606.
...
DO IF(Has_Orders = 1).
   ...
   COMPUTE ORD_PM = (SUM(ORD_0711 TO ORD_0616) / 60).
...
*Flag the accounts whose LN of Orders per Month is > 5.5.
COMPUTE Order_Outlier = (ORD_PM_LN > 5.5).
FORMATS Order_Outlier(F1.0).

*Flag the accounts whose LN of std deviation is >= 0.5.
COMPUTE Stable_Order_Outlier = (ORD_SD_LN >= 0.5).
FORMATS Stable_Order_Outlier(F1.0).

*Remove records that do not have orders as well as order outliers.
COMPUTE Include = (Has_Orders = 1 AND Order_Outlier = 0 AND Stable_Order_Outlier = 0).
FORMATS Include(F1.0).
EXE.

*Save the resulting file.
SAVE OUTFILE = '/usr/spss/userdata/Albrecht/Display Geo/Five Year Orders through June16 by Account from TD.sav'.
...
RENAME VARIABLES K_1 = Group1 K_2 = Group2.

DATE M 7 12 Y 2011.

*Group 1 Forecast.
USE year 2011 month 7 THRU year 2016 month 6.
